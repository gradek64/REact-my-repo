...
if (localStorage.getItem('theme')) {
  ...
  document.body.classList.remove('dark', 'solarized', 'light', 'solarized-dark');
...
var toggleSolarized = document.querySelector('.toggle-solarized');
var toggleSolarizedDark = document.querySelector('.toggle-solarized-dark');
...
toggleLight.addEventListener('click', function(e) {
  ...
  document.body.classList.remove('dark', 'solarized', 'solarized-dark');
  localStorage.setItem('theme', 'light');
});
toggleDark.addEventListener('click', function(e) {
  e.preventDefault();
  document.body.classList.add('dark');
  document.body.classList.remove('solarized', 'solarized-dark');
  localStorage.setItem('theme', 'dark');
});
toggleSolarized.addEventListener('click', function(e) {
  e.preventDefault();
...
toggleSolarizedDark.addEventListener('click', function(e) {
  e.preventDefault();
...
var tweetItBtn = document.querySelector('.tweet-it');
...
var algoliaInput = document.querySelector('.algolia__input');
...
algoliaInput.addEventListener('keypress', function(e) {
    var key = e.which || e.keyCode;
    if (key === 13) {
      algoliaInput.blur();
      document.querySelector('.top-bar').className = 'top-bar';
    }
    if (key === 27) {
      algoliaInput.blur();
      algoliaInput.value = '';
...
algoliaInput.addEventListener('focus', loadAlgoliaScripts);
algoliaInput.addEventListener('mouseover', loadAlgoliaScripts);
var loadedScripts = [];

function loadAlgoliaScripts() {
  loadScript('/assets/js/algoliasearch/3.24.11/algoliasearchLite.min.js');

  whenAvailable('algoliasearch', function() {
    loadScript('/assets/js/algoliasearch.helper/2.24.0/algoliasearch.helper.min.js');
  });
  loadScript('/assets/js/hogan.js/3.0.2/hogan.min.js');

  whenAvailable("algoliasearchHelper", function(t) {
    loadScript('/assets/js/algolia2.js');
  });
}
...
function loadScript(url) {
  if (!loadedScripts.includes(url)) {
    var isLoaded = document.querySelectorAll('.algolia');
    if(isLoaded.length > 3) {
      algoliaInput.removeEventListener('focus', loadAlgoliaScripts);
      algoliaInput.removeEventListener('mouseover', loadAlgoliaScripts);
      return;
    }
    var myScript = document.createElement("script");
    myScript.src = url;
    myScript.className = 'algolia';
...
var savedPostsLink = document.querySelector('.saved-link');
if(window.localStorage && savePost && savedPostsLink) {
  var postTitle = document.querySelector('h1').textContent;
  var saved = false;
  // savePost.style.visibility = 'visible';
  savePost.style.opacity = 1;

  if (
      (!localStorage.getItem('algoliasearch-client-js') && window.localStorage.length) ||
      (localStorage.getItem('algoliasearch-client-js') && window.localStorage.length > 1)
    ) {
    savedPostsLink.style.opacity = 1;
  }

  if (!localStorage.getItem('post ' + window.location.href)) {
  ...
  savePost.addEventListener('click', function(e) {
    e.preventDefault();
    if (!saved) {
      localStorage.setItem('post ' + window.location.href, postTitle);
      savePost.innerText = '✓ Saved';
      if (savedPostsLink.style.display === '') {
        savedPostsLink.style.display = 'block';
      }
      saved = true;
    } else {
      localStorage.removeItem('post ' + window.location.href);
      savePost.innerText = 'Save';
      if (!window.localStorage.length) {
        savedPostsLink.style.display = '';
      }
      saved = false;
    }
  });
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/sw3.js').catch(function(err) {
